  ...
  if (status !== "connected") {
    return (
      <div className="p-4 flex flex-col gap-4 w-[350px] h-[400px] justify-center items-center bg-background text-foreground">
        ...
        <p className="text-muted-foreground text-center text-sm">Connect your controller to access your realms.</p>
    ...
    <div className="p-4 flex flex-col gap-4 w-[400px] h-[500px] overflow-y-auto bg-background text-foreground">
      <div className="flex justify-between items-center bg-secondary p-2 rounded-md">
        <div className="flex flex-col">
          ...
          <span className="font-mono text-xs truncate w-32" title={address}>
            {address}
          </span>
        </div>
...
const RealmsList = ({ account }: { account: AccountInterface | undefined }) => {
  ...
  useEffect(() => {
    ...
    const myStructures = Array.from(structures)
      .map((entityId) => {
        ...
        <Card key={i} className="p-3 flex flex-col gap-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs">
              X:{realm.base?.coord_x} Y:{realm.base?.coord_y}
            </span>
          </div>
          <div className="flex gap-2 justify-end">
            <Button size="sm" onClick={() => buildWorkerHut(realm.entity_id)}>
              Build Hut
            </Button>
            <Button size="sm" variant="secondary" onClick={() => deployArmy(realm.entity_id)}>
              Deploy Army
            </Button>
          </div>
        </Card>
